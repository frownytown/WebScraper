...
def fictionpress_loop_url(number_of_chapters):
	...
	while loop_count <= number_of_chapters:
		fiction_list_of_urls.append(fictionpress_url_part_1 + str(loop_count) + fictionpress_url_part_3)
		loop_count += 1
		print(fictionpress_url_part_1 + str(loop_count) + fictionpress_url_part_3 + " added", file=sys.stdout)
	...
	try:
		if location_selection == "royal":
			...
			for url in list_of_urls:
				get_page_content((domain_name + url), royalroad_div, royalroad_attrs_1, royalroad_attrs_2)
				i += 1
...
if __name__ == '__main__':
	...
	print(sys.argv)
	location = selector_function(sys.argv[1])
	try:
		if location == "royal":
